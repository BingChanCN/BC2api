...
type AccountQuery struct {
	...
	withManagedProxyLease *ManagedProxyLeaseQuery
...
// QueryManagedProxyLease chains the current query on the "managed_proxy_lease" edge.
func (_q *AccountQuery) QueryManagedProxyLease() *ManagedProxyLeaseQuery {
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withManagedProxyLease: _q.withManagedProxyLease.Clone(),
...
// WithManagedProxyLease tells the query-builder to eager-load the nodes that are connected to
// the "managed_proxy_lease" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithManagedProxyLease(opts ...func(*ManagedProxyLeaseQuery)) *AccountQuery {
	query := (&ManagedProxyLeaseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withManagedProxyLease = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withManagedProxyLease; query != nil {
		if err := _q.loadManagedProxyLease(ctx, query, nodes, nil,
			func(n *Account, e *ManagedProxyLease) { n.Edges.ManagedProxyLease = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadManagedProxyLease(ctx context.Context, query *ManagedProxyLeaseQuery, nodes []*Account, init func(*Account), assign func(*Account, *ManagedProxyLease)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(managedproxylease.FieldAccountID)
	}
	query.Where(predicate.ManagedProxyLease(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ManagedProxyLeaseColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
