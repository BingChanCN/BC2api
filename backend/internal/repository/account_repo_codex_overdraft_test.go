...
func TestPersistCodexQuotaOverdraftProbeUnlessFailedPreservesTerminalFailure(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE accounts.*cycle_key}', ''\) = ''.*cycle_key}', ''\) = \$4.*status}', ''\) <> 'failed'`).
...
func TestPersistCodexQuotaOverdraftProbeUnlessFailedRejectsStaleCycle(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	...
	state := &service.CodexQuotaOverdraftProbeState{
		Status:      "passed",
		CycleKey:    "5h:old-cycle",
		QuotaWindow: "5h",
		Limit:       1,
		StartedAt:   now,
	}
	// Non-failure writes must bind the incoming cycle as equality, never
	// `cycle_key <> incoming`. A stale cycle therefore cannot overwrite a
	// newer failed/pending row.
	mock.ExpectExec(`(?s)UPDATE accounts.*cycle_key}', ''\) = ''.*cycle_key}', ''\) = \$4.*status}', ''\) <> 'failed'`).
		WithArgs(service.CodexQuotaOverdraftProbeExtraKey, sqlmock.AnyArg(), int64(77), state.CycleKey).
		WillReturnResult(sqlmock.NewResult(0, 0))
