...
func registerRoutes(
	...
	pluginRegistry := pluginruntime.NewRegistry(cfg.Plugins)
	pluginRuntime := pluginruntime.NewRuntime(pluginRegistry)
	if h.GameLedger != nil {
		h.GameLedger.SetPluginRegistry(pluginRegistry)
	}
	...
	routes.RegisterPluginRoutes(r, v1, pluginRuntime, jwtAuth, adminAuth, auditLog, settingService, panelRateLimiter, h.GameLedger)
