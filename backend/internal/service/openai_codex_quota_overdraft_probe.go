...
type codexQuotaOverdraftTransportNotifier interface {
	notifyManagedProxyTransportFailure(context.Context, *Account, error)
	notifyManagedProxyTransportSuccess(context.Context, *Account)
}
...
type CodexQuotaOverdraftCoordinator struct {
	...
	transportNotifier   codexQuotaOverdraftTransportNotifier
...
func NewCodexQuotaOverdraftCoordinator(
	...
	coordinator.transportNotifier, _ = runtimeBlocker.(codexQuotaOverdraftTransportNotifier)
	return coordinator
}
...
func (c *CodexQuotaOverdraftCoordinator) HandleQuota429(
	...
	if consumeCodexQuotaOverdraftInjection(ctx, account.ID) {
...
func (c *CodexQuotaOverdraftCoordinator) runProbeAttempt(ctx context.Context, account *Account, model string) codexQuotaOverdraftProbeResult {
	...
	if err != nil {
		if c.transportNotifier != nil {
			c.transportNotifier.notifyManagedProxyTransportFailure(ctx, account, err)
		}
	...
	if readErr != nil {
		if c.transportNotifier != nil {
			c.transportNotifier.notifyManagedProxyTransportFailure(ctx, account, readErr)
		}
		result.Status, result.ReasonCode = "inconclusive", "invalid_response"
		return result
	}
	if c.transportNotifier != nil {
		c.transportNotifier.notifyManagedProxyTransportSuccess(ctx, account)
	}
	if len(body) > codexQuotaOverdraftProbeBodyLimit {
		result.Status, result.ReasonCode = "inconclusive", "invalid_response"
		return result
	}
	result.Body = body
