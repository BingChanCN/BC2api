...
type codexOverdraftSnapshotCacheStub struct {
	SchedulerCache
	accounts []*Account
	hit      bool
}

func (s *codexOverdraftSnapshotCacheStub) GetSnapshot(context.Context, SchedulerBucket) ([]*Account, bool, error) {
	return s.accounts, s.hit, nil
}

type codexOverdraftSchedulingRepoStub struct {
	schedulerTestOpenAIAccountRepo
	listCalls int
}

func (r *codexOverdraftSchedulingRepoStub) ListSchedulableByPlatform(ctx context.Context, platform string) ([]Account, error) {
	r.listCalls++
	return r.schedulerTestOpenAIAccountRepo.ListSchedulableByPlatform(ctx, platform)
}

func (r *codexOverdraftSchedulingRepoStub) ListSchedulableByGroupIDAndPlatform(ctx context.Context, groupID int64, platform string) ([]Account, error) {
	r.listCalls++
	return r.schedulerTestOpenAIAccountRepo.ListSchedulableByGroupIDAndPlatform(ctx, groupID, platform)
}

func (r *codexOverdraftSchedulingRepoStub) ListSchedulableUngroupedByPlatform(ctx context.Context, platform string) ([]Account, error) {
	r.listCalls++
	return r.schedulerTestOpenAIAccountRepo.ListSchedulableUngroupedByPlatform(ctx, platform)
}

func TestCodexQuotaOverdraftSchedulingFallsBackOnlyAfterSnapshotExhaustion(t *testing.T) {
	...
	overdraft := Account{
		...
		Schedulable: true,
		Extra: map[string]any{
			"codex_5h_used_percent": 96,
			"codex_5h_reset_at":     now.Add(time.Hour).Format(time.RFC3339),
		},
	}
	repo := &codexOverdraftSchedulingRepoStub{
		schedulerTestOpenAIAccountRepo: schedulerTestOpenAIAccountRepo{accounts: []Account{overdraft}},
	}
	cache := &codexOverdraftSnapshotCacheStub{accounts: []*Account{normal}, hit: true}
	...
	require.Equal(t, normal.ID, accounts[0].ID)
	...
	require.NoError(t, err)
	require.Len(t, accounts, 1)
	require.Equal(t, overdraft.ID, accounts[0].ID)
...
func TestCodexQuotaOverdraftInjection(t *testing.T) {
	...
	oauth.Extra["codex_5h_used_percent"] = 94
	nextTurn := svc.prepareCodexQuotaOverdraftBody(ctx, oauth, false, body)
	require.Equal(t, string(body), string(nextTurn), "后续不满足条件的轮次不能注入")
	require.False(t, codexQuotaOverdraftWasInjected(ctx, oauth.ID), "后续轮次不能继承上一轮注入标记")
}

func TestBeginCodexQuotaOverdraftTurnClearsPriorTurnInjection(t *testing.T) {
	ctx := WithCodexQuotaOverdraftScheduling(context.Background())
	const accountID int64 = 77

	markCodexQuotaOverdraftInjected(ctx, accountID)
	require.True(t, codexQuotaOverdraftWasInjected(ctx, accountID))

	BeginCodexQuotaOverdraftTurn(ctx, accountID)
	require.False(t, codexQuotaOverdraftWasInjected(ctx, accountID))

	markCodexQuotaOverdraftInjected(ctx, accountID)
	require.True(t, consumeCodexQuotaOverdraftInjection(ctx, accountID))
	require.False(t, consumeCodexQuotaOverdraftInjection(ctx, accountID), "注入标记只能消费一次")
}
