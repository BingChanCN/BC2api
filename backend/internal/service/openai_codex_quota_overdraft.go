...
func consumeCodexQuotaOverdraftInjection(ctx context.Context, accountID int64) bool {
	if accountID <= 0 {
		return false
	}
	state := codexQuotaOverdraftRequestStateFromContext(ctx)
	...
	_, loaded := state.injectedAccounts.LoadAndDelete(accountID)
	return loaded
}

// BeginCodexQuotaOverdraftTurn clears request-local state left by a previous
// turn on a long-lived WebSocket connection. Injection is decided per upstream
// request, so a later turn must not inherit an earlier turn's marker.
func BeginCodexQuotaOverdraftTurn(ctx context.Context, accountID int64) {
	if accountID <= 0 {
		return
	}
	state := codexQuotaOverdraftRequestStateFromContext(ctx)
	if state != nil {
		state.injectedAccounts.Delete(accountID)
	}
}
...
func (s *OpenAIGatewayService) prepareCodexQuotaOverdraftBody(ctx context.Context, account *Account, compact bool, body []byte) []byte {
	if account != nil {
		BeginCodexQuotaOverdraftTurn(ctx, account.ID)
	}
