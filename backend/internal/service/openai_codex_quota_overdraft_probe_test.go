...
type codexOverdraftTransportNotifierStub struct {
	successCalls int
	failureCalls int
	account      *Account
	err          error
...
func (n *codexOverdraftTransportNotifierStub) notifyManagedProxyTransportFailure(_ context.Context, account *Account, err error) {
	n.failureCalls++
	n.account = account
	n.err = err
}

func (n *codexOverdraftTransportNotifierStub) notifyManagedProxyTransportSuccess(_ context.Context, account *Account) {
	n.successCalls++
	n.account = account
}
...
func TestCodexQuotaOverdraftProbeUsesManagedProxyAndNotifiesTransportRuntime(t *testing.T) {
	now := time.Now().UTC()
	...
	account.Proxy = &Proxy{
		ID:       proxyID,
		Protocol: "socks5h",
		Host:     "managed-proxy.example.com",
		Port:     1080,
		Username: "derived-session",
	...
	t.Run("success", func(t *testing.T) {
		...
		notifier := &codexOverdraftTransportNotifierStub{}
		...
		require.Equal(t, []string{account.Proxy.URL()}, upstream.proxyURLs)
		require.Equal(t, 1, notifier.successCalls)
		require.Zero(t, notifier.failureCalls)
		require.Same(t, account, notifier.account)
	})

	t.Run("failure", func(t *testing.T) {
		transportErr := errors.New("proxyconnect tcp: 407 Proxy Authentication Required")
		upstream := &queuedHTTPUpstream{errs: []error{transportErr}}
		notifier := &codexOverdraftTransportNotifierStub{}
		coordinator := &CodexQuotaOverdraftCoordinator{
			httpUpstream:      upstream,
			transportNotifier: notifier,
		...
		require.Equal(t, []string{account.Proxy.URL()}, upstream.proxyURLs)
		require.Zero(t, notifier.successCalls)
		require.Equal(t, 1, notifier.failureCalls)
		require.Same(t, account, notifier.account)
		require.ErrorIs(t, notifier.err, transportErr)
	})
}
...
func TestCodexQuotaOverdraftInjectedBusinessQuota429FailsImmediately(t *testing.T) {
	...
	require.Equal(t, 1, repo.tempPauseCalls)
	require.False(t, codexQuotaOverdraftWasInjected(ctx, account.ID), "终态额度失败必须消费本次注入标记")
}

func TestCodexQuotaOverdraftResponseFailedUsesBusinessFailurePath(t *testing.T) {
	now := time.Date(2026, time.August, 13, 14, 0, 0, 0, time.UTC)
	account := newCodexOverdraftProbeTestAccount(now)
	repo := &codexOverdraftProbeRepoStub{account: account}
	coordinator := &CodexQuotaOverdraftCoordinator{
	...
	svc := &OpenAIGatewayService{codexQuotaOverdraft: coordinator}
	ctx := WithCodexQuotaOverdraftScheduling(context.Background())
	markCodexQuotaOverdraftInjected(ctx, account.ID)
	payload := []byte(`{"type":"response.failed","response":{"error":{"code":"usage_limit_reached","message":"usage limit reached"}}}`)

	handled := svc.handleCodexQuotaOverdraftResponseFailed(ctx, account, nil, payload, "gpt-5.4")
	...
	require.Equal(t, "business_quota_limited", state.ReasonCode)
	require.Equal(t, 1, repo.tempPauseCalls)
	require.False(t, codexQuotaOverdraftWasInjected(ctx, account.ID))
}

func TestOpenAIStreamingQuotaResponseFailedUsesOverdraftStateMachine(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			...
			`data: {"type":"response.failed","response":{"error":{"code":"usage_limit_reached","message":"usage limit reached"}}}`,
			"",
		}, "\n"))),
	}

	_, err := svc.handleStreamingResponse(ctx, resp, c, account, time.Now(), "gpt-5.4", "gpt-5.4")
...
func TestOpenAIChatCompletionsQuotaResponseFailedUsesOverdraftStateMachine(t *testing.T) {
	gin.SetMode(gin.TestMode)
	now := time.Date(2026, time.August, 13, 14, 0, 0, 0, time.UTC)
	account := newCodexOverdraftProbeTestAccount(now)
	repo := &codexOverdraftProbeRepoStub{account: account}
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			"event: response.failed",
			`data: {"type":"response.failed","response":{"status":"failed","error":{"code":"usage_limit_reached","message":"usage limit reached"}}}`,
			"",
		}, "\n"))),
	}

	_, err := svc.handleChatStreamingResponse(resp, c, account, "gpt-5.4", "gpt-5.4", "gpt-5.4", time.Now(), 16)
...
func TestOpenAIMessagesQuotaResponseFailedUsesOverdraftStateMachine(t *testing.T) {
	gin.SetMode(gin.TestMode)
	now := time.Date(2026, time.August, 13, 14, 0, 0, 0, time.UTC)
	account := newCodexOverdraftProbeTestAccount(now)
	repo := &codexOverdraftProbeRepoStub{account: account}
...
func TestOpenAIWSTerminalQuotaFailureUsesOverdraftStateMachine(t *testing.T) {
	now := time.Date(2026, time.August, 13, 14, 0, 0, 0, time.UTC)
	account := newCodexOverdraftProbeTestAccount(now)
	repo := &codexOverdraftProbeRepoStub{account: account}
	coordinator := &CodexQuotaOverdraftCoordinator{
	...
	terminal := svc.handleOpenAIWSTerminalTransientFailure(ctx, account, "gpt-5.4", nil, payload)
